...
class AgentLogic extends BaseLogic {
    ...
    public function index($request){
        ...
        foreach($list as $i=>$iList){
            $list[$i]=stringChange('formatMoneyByArray',$list[$i],array('money','fl'));
        }
    ...
    public function edit($request){
        ...
        $row=stringChange('formatMoneyByArray',$row,array('money','fl'));
        $params = array(
            'edit' => $row,
            'act'=>'edit',
    ...
    public function fandianupdate(){
        //订单日期 获取最早的
        $orderBuffer=SM('Dingdan')->findData('*','status=1','ddid asc');
        $lastfd=strtotime(date('Y-m-d',$orderBuffer['paytime']));
        ...
        $addFandian=array();
        for($i=0;$i<$count;$i++){
            $buffer=$dingdan->getAgentMoney($lastfd,$lastfd+24*3600);
            if($buffer){
    ...
    public function fandian($request){
        ...
        foreach($list as $i=>$iList){
            ...
            $list[$i]=stringChange('formatMoneyByArray',$list[$i],array('totalmoney','havemoney','fl'));
        }
